...
const candidateMarketingController = require("../controllers/candidateMarketingController"); // Adjust the path accordingly
...
router.get("/candidatemarketing/users", AdminValidationMiddleware, (req, res) => {
  const db = req.db;
  ...
  const query = `
    ...
    UNION 
    SELECT DISTINCT c.id as id, CONCAT(c.name, '-', c.email) as name 
    FROM candidate c
    ORDER BY name;
  `;
...
// Route to get all candidate marketing entries with pagination and search
router.get("/candidatemarketing", AdminValidationMiddleware, (req, res) => {
  const db = req.db;
  if (!db) {
    return res.status(500).json({ message: "Database connection error" });
  }
...
// Route to insert a new candidate marketing entry
// router.post("/candidatemarketing/insert", (req, res) => {
//   const newCandidateMarketing = req.body;

//   // Insert the new candidate marketing record
//   req.db.query("INSERT INTO candidatemarketing SET ?", newCandidateMarketing, (err, results) => {
//     if (err) {
//       console.error("Database insert error:", err);
//       return res.status(500).json({ message: "Database error" });
//     }
//     res.status(201).json({ id: results.insertId, ...newCandidateMarketing });
//   });
// });
